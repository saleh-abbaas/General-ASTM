...
from ahg_lis_project.config import AHGLISConfig, device_slug
...
class ASTMListener:
    ...
    def __init__(self, config: AHGLISConfig) -> None:
        self.config = config
        self._device_label = config.device_name or "Device"
        self._device_slug = device_slug(self._device_label)
    ...
    def run(self, stop_event: Optional[threading.Event] = None) -> None:
        ...
        LOGGER.info(
            "Starting AHG LIS listener for %s on %s",
            self._device_label,
        ...
        while not (self._stop_event.is_set() or external_stop.is_set()):
            try:
                ...
                LOGGER.error("Serial error on %s: %s", self._device_label, exc)
                self._close_port()
                if self._stop_event.wait(5) or (stop_event and stop_event.wait(5)):
                    break
            except Exception as exc:  # pragma: no cover - defensive logging
                LOGGER.exception(
                    "Unexpected error while reading from %s: %s",
                    self._device_label,
                    exc,
                )
                time.sleep(1)
        self._shutdown()
        LOGGER.info("Listener for %s stopped", self._device_label)

    # ------------------------------------------------------------------
    # Internal helpers
    # ------------------------------------------------------------------
    def _open_port(self) -> serial.Serial:
        LOGGER.debug("Opening serial port %s for %s", self.config.serial_port, self._device_label)
        port = serial.Serial(self.config.serial_port, baudrate=self.config.baudrate, timeout=1)
        LOGGER.info("Connected to %s for %s", self.config.serial_port, self._device_label)
        return port
    ...
    def _handle_byte(self, byte: bytes) -> None:
        LOGGER.debug("[%s] Received byte 0x%s", self._device_label, byte.hex())
        if byte == self.ENQ:
            self._ack()
            self._prepare_file()
            self._buffer = [byte.decode("latin1")]
            LOGGER.info("[%s] <ENQ> received - session started", self._device_label)
            return
        ...
        if byte == self.LF:
            ...
            self._write_buffer()
            LOGGER.info(
                "[%s] <LF> received - chunk written to %s",
                self._device_label,
                self._current_file,
            )
            return
        ...
        if byte == self.EOT:
            ...
            self._write_buffer()
            self._close_file()
            LOGGER.info("[%s] <EOT> received - session finished", self._device_label)
            return
    ...
    def _prepare_file(self) -> None:
        ...
        filename = f"{timestamp}_{self._device_slug}.txt"
        self._current_file = Path(self.config.output_folder) / filename
